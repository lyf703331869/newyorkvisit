...
export const ItineraryCard = ({ title, days, theme, highlight, plan }: Itinerary) => (
  <article className="card-surface p-5 space-y-3">
    ...
    <ul className="mt-3 space-y-2 text-xs text-slate-600">
      {plan.map((item) => (
        <li key={item} className="rounded-2xl bg-slate-50 px-3 py-2 text-slate-700">
          {item}
        </li>
      ))}
    </ul>
    <button className="w-full rounded-full border border-slate-200 py-2 text-sm text-slate-800">
      下载行程 PDF（预留）
    </button>
  </article>
);
